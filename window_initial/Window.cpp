...
WindowManager::WindowManager()
{
    ...
    wcex.lpszMenuName  = 0;
    wcex.lpszClassName = UTF16ToUTF8(mWndClass).c_str();
...
{
    Close();
    UnregisterClass(UTF16ToUTF8(mWndClass).c_str(), mInstance);
}
...
bool WindowManager::Open()
{
    ...
    if (mFullscreen)
    {
        mHandle = CreateWindowEx(gFullscreenExStyle, UTF16ToUTF8(mWndClass).c_str(), mTitle.c_str(),
    ...
    else
    {
        ...
        mHandle = CreateWindowEx(gWindowedExStyle,
                                 UTF16ToUTF8(mWndClass).c_str(), mTitle.c_str(),
                                 gWindowedStyle,
                                 mTop, mTop,
                                 WindowRect.right - WindowRect.left,
                                 WindowRect.bottom - WindowRect.top,
