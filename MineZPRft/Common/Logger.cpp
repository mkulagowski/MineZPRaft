                 ...
                 int line, const std::string& message)
{
    ...
    // Modify it to look like hh:mm:ss - easier to understand,
    // after running for some time.
    // Note: If hh > 99, then it's length is properly adjusted.
    double timeElapsed = mTimer.Stop();
    int hh = static_cast<int>(timeElapsed / 3600.0);
    int mm = static_cast<int>(timeElapsed / 60.0) % 60;
    int ss = static_cast<int>(fmod(timeElapsed, 60));
    int hhLen = hh > 99 ? static_cast<int>(log10(hh)) + 1 : 2;
    std::stringstream timeElapsedStream;
    timeElapsedStream << std::setfill('0') << std::setw(hhLen) << hh << ":"
                      << std::setfill('0') << std::setw(2) << mm << ":"
                      << std::setfill('0') << std::setw(2) << ss;

    // Create output message with the following format:
    // <time elapsed> [<type>] <file> <func>@<line>: <msg>
    std::string output = timeElapsedStream.str() +" [" + LogTypeToString(type)
        + "] " + fileStr + " " + func + "@" + std::to_string(line) + ": "
        + message;
