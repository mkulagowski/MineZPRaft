...
bool WindowManager::Open()
{
    ...
    if (glXCreateContextAttribsARB)
    {
        ...
        {
            // TODO log warning
            std::cerr << "GL 3.3 core profile not acquired. Falling back to old profile." << std::endl;
            std::cerr << "Keep in mind, the renderer MIGHT NOT WORK due to too old OGL version!" << std::endl;
    ...
    {
        // TODO log warning
        std::cerr << "glXCreateContextAttribsARB not available. Creating OGL context the old way." << std::endl;
        std::cerr << "Keep in mind, the renderer MIGHT NOT WORK due to too old OGL version!" << std::endl;
        mContext = glXCreateNewContext(mDisplay, bestFB, GLX_RGBA_TYPE, NULL, GL_TRUE);
        if (!mContext)
        {
